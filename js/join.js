...
function isTxt(el,len) {
    ...
    if(txt.length >= len){ // value값이 조건에 부합한다면
        // 중복 오류메시지 방지
        const errMsgs = input.closest("td").querySelectorAll("p");
        if(errMsgs.length > 0) input.closest("td").querySelector("p").remove();
        return true;
    }else{ // value값이 조건에 부합하지 않는다면
        const errMsgs = input.closest("td").querySelectorAll("p");
...
function isPwd(el1, el2, len){
    ...
    if(pwd1_val===pwd2_val && pwd1_val.length>=len && num.test(pwd1_val) && eng.test(pwd1_val) && spc.test(pwd1_val)){
        ...
        const errMsg = document.createElement("p");
        errMsg.append(`Please enter ${len} or more letters with at least one numeric and special character.`);
